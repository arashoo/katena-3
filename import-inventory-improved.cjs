...
function createGlassEntries(inventoryItem) {
    ...
    // Create individual glass entries based on quantity - but limit to reasonable amounts
    const actualQuantity = Math.min(quantity, 300); // Cap at 300 pieces per line item
    
    for (let i = 0; i < actualQuantity; i++) {
        // Create a more readable ID
        const timestamp = Date.now();
        const glassId = `${width}x${height}_${color.replace(/\s+/g, '_').toLowerCase()}_${timestamp}_${i}`;
...
function convertInventoryToJSON() {
    ...
    console.log('Processing inventory data (improved version)...');
    ...
    lines.forEach((line, index) => {
        ...
        if (inventoryItem) {
            ...
            console.log(`Processed: ${inventoryItem.width}x${inventoryItem.height} ${inventoryItem.color} - ${Math.min(inventoryItem.quantity, 300)} pieces - Project: ${inventoryItem.project || 'Available'}`);
        }
    });
    
    console.log(`\nSummary:`);
    console.log(`- Processed ${totalProcessed} inventory lines`);
    console.log(`- Created ${allGlasses.length} individual glass entries`);
    console.log(`- Original total: ${totalGlassCount} pieces`);
...
const glassesFilePath = path.join(__dirname, 'backend', 'data', 'glasses.json');
...
const deploymentGlassesPath = path.join(__dirname, 'deployment-files', 'data', 'glasses.json');
if (fs.existsSync(path.dirname(deploymentGlassesPath))) {
    fs.writeFileSync(deploymentGlassesPath, JSON.stringify(glassInventory, null, 2));
    console.log(`📁 Also updated deployment file: ${deploymentGlassesPath}`);
}
...
console.log('\n📊 Project allocation:');
Object.entries(projectCounts).sort((a, b) => b[1] - a[1]).slice(0, 15).forEach(([project, count]) => {
    console.log(`  ${project}: ${count} pieces`);
});

console.log('\n🔧 File size optimization: Capped large quantities to keep JSON manageable');